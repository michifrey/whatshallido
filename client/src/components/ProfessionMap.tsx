...
export function ProfessionMap({ professions, selected, onSelect }: Props) {
  const [imgOk, setImgOk] = useState(true);
  const counts = useMemo(() => zoneCounts(professions), [professions]);
          ...
          return (
            ...
            >
              <span className="relative flex flex-col items-center">
                {/* Pin */}
                <span
                  style={{ borderColor: active ? z.color : "#ffffff", boxShadow: active ? `0 0 0 4px ${z.color}55` : undefined }}
                ...
                >
                  <span className="block text-xs font-extrabold" style={{ color: z.color }}>
                    {z.emoji} {z.short}
                  </span>
                  ...
                  <span className="mt-1.5 block text-[11px] font-bold text-slate-800 dark:text-slate-100">
                    {counts[z.key] ?? 0} Berufe · zum Öffnen klicken →
                  </span>
                </span>
              </span>
            </button>
        ...
        {ZONES.map((z) => {
          ...
          return (
            <button
              key={z.key}
              onClick={() => onSelect(active ? "" : z.key)}
              style={{ borderColor: active ? z.color : "transparent" }}
              className="relative overflow-hidden rounded-2xl border-[3px] bg-white p-3 text-center shadow-card dark:bg-slate-900"
            >
              <span className="absolute inset-x-0 top-0 h-1.5" style={{ background: z.color }} />
              <span className="block text-3xl">{z.emoji}</span>
              <span className="mt-1 block text-sm font-extrabold leading-tight">{z.short}</span>
              <span className="mt-0.5 block text-[11px] leading-snug text-slate-500 dark:text-slate-400">{z.desc}</span>
              <span className="mt-1 block text-[11px] font-bold text-slate-600 dark:text-slate-300">{counts[z.key] ?? 0} Berufe</span>
            </button>
          );
        })}
      </div>
